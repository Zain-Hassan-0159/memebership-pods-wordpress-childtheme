<?php
/* 
*   Template Name: Portal Navigation
*   Post Type: page
*/
...
if($course_id !== ""){
  ...
  set_transient( 'recent_course_id_' . get_level_id_of_current_user() , $course_id, 60 * DAY_IN_SECONDS );
    ...
    $navigationsArray = $pod->field("select_navigations_template");
    $navigationsId = $navigationsArray["ID"];
    $navigationsPod = pods("navigations", $navigationsId);
    $navigationsPdfs = $navigationsPod->field("pdf_upload");

    // print_r($navigationsPdf);
    // exit;
...
?>
    ...
    <section class="meal-plan">
        ...
        <div class="how-to-use-plan">
            <h2><?php echo !empty($navigationsArray) ? $navigationsArray['post_title'] : ""; ?></h2>
            <div class="meal-plan-btn">
              <?php
              foreach($navigationsPdfs as $navigationsPdf){
                ?>
                <a href="<?php echo $navigationsPdf['guid']; ?>" download class="meal-plan-button"><?php echo $navigationsPdf['post_title']; ?><img src="<?php echo get_stylesheet_directory_uri()?>/assets/images/download.png" alt="" ></a>
                <?php
              }
              ?>
            </div>
        ...
        <div class="container">
        <?php echo !empty($navigationsArray) ? $navigationsArray['post_content'] : ""; ?>
        </div>
    </section>
