<?php
/* 
*   Template Name: Portal Instructions
*   Post Type: page
*/
...
if($course_id !== ""){
  ...
  set_transient( 'recent_course_id_' . get_level_id_of_current_user() , $course_id, 60 * DAY_IN_SECONDS );
    ...
    $instructionsArray = $pod->field("select_instructions_template");
    $instructionsId = $instructionsArray["ID"];
    $instructionsPod = pods("instructions", $instructionsId);
    $instructionsPdfs = $instructionsPod->field("pdf_upload");

    // print_r($instructionsPdf);
    // exit;
...
?>
    ...
    <section class="meal-plan">
        ...
        <div class="how-to-use-plan">
            <h2><?php echo !empty($instructionsArray) ? $instructionsArray['post_title'] : ""; ?></h2>
            <div class="meal-plan-btn">
              <?php
              foreach($instructionsPdfs as $instructionsPdf){
                ?>
                <a href="<?php echo $instructionsPdf['guid']; ?>" download class="meal-plan-button"><?php echo $instructionsPdf['post_title']; ?><img src="<?php echo get_stylesheet_directory_uri()?>/assets/images/download.png" alt="" ></a>
                <?php
              }
              ?>
            </div>
        ...
        <div class="container">
        <?php echo !empty($instructionsArray) ? $instructionsArray['post_content'] : ""; ?>
        </div>
    </section>
