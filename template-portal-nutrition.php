<?php
/* 
*   Template Name: Portal Nutrition
*   Post Type: page
*/
...
if($course_id !== ""){
  ...
  set_transient( 'recent_course_id_' . get_level_id_of_current_user() , $course_id, 60 * DAY_IN_SECONDS );
    ...
    $nutritonArray = $pod->field("select_nutrition_template");
    $nutritonId = $nutritonArray["ID"];
    $nutritionPod = pods("nutrition", $nutritonId);
    $nutritionPdfs = $nutritionPod->field("pdf_upload");

    // print_r($nutritionPdf);
    // exit;
...
?>
    ...
    <section class="meal-plan">
        ...
        <div class="how-to-use-plan">
            <h2><?php echo !empty($nutritonArray) ? $nutritonArray['post_title'] : ""; ?></h2>
            <div class="meal-plan-btn">
              <?php
              foreach($nutritionPdfs as $nutritionPdf){
                ?>
                <a href="<?php echo $nutritionPdf['guid']; ?>" download class="meal-plan-button"><?php echo $nutritionPdf['post_title']; ?><img src="<?php echo get_stylesheet_directory_uri()?>/assets/images/download.png" alt="" ></a>
                <?php
              }
              ?>
            </div>
        ...
        <div class="container">
        <?php echo !empty($nutritonArray) ? $nutritonArray['post_content'] : ""; ?>
        </div>
    </section>
